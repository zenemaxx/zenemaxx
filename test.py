...
#kick
@Bot.command( pass_context = True )
@commands.has_any_role("⚒ Красная администрация ⚒","❖ Developer Discord ❖","Старший модератор Discord™","Модератор Discord™" )
...
#ban
@Bot.command( pass_context = True)
@commands.has_any_role("⚒ Красная администрация ⚒","❖ Developer Discord ❖","Старший модератор Discord™" )
...
#unmute
@Bot.command()
@commands.has_any_role("⚒ Красная администрация ⚒","❖ Developer Discord ❖","Старший модератор Discord™","Модератор Discord™","Младший модератор Discord™" )
...
async def role(ctx, role: str ):
	...
	for rol in roli:
		if rol.id == EXROLE: #ЕСЛИ РОЛЬ = EVERYONE =>
			...
			k = k+1
	if k < 3:
		if str(role) != "BOT" and str(role) != "⚒ Красная администрация ⚒" and str(role) != "":
